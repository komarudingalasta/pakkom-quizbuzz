...
function beep(id=1){try{let A=window.AudioContext||window.webkitAudioContext;if(!A)return;let c=new A(),o=c.createOscillator(),g=c.createGain(),n=(id-1)%SOUNDS.length;o.type=["sine","square","triangle","sawtooth"][n%4];o.frequency.value=360+(n*73);g.gain.setValueAtTime(.0001,c.currentTime);g.gain.exponentialRampToValueAtTime(.28,c.currentTime+.02);g.gain.exponentialRampToValueAtTime(.0001,c.currentTime+.34);o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.36);setTimeout(()=>c.close(),500)}catch(e){}}
