...
const SOUNDS=[
 ["🔔","Ding"],["📣","Horn"],["🥁","Drum"],["🚨","Alarm"],["🛎️","Bell"],["🎺","Trumpet"],
 ["🔊","Beep"],["🪇","Shake"],["🎵","Tone"],["📯","Call"],["🎹","Piano"],["🫧","Pop"],
 ["⚡","Zap"],["✨","Spark"],["🌀","Sweep"],["🎯","Hit"]
];
...
function beep(id=1){
  try{
    ...
    // 16 punchy game-show style signatures.
    const patterns=[
      [660,880,1175], [440,660,990], [520,780,1040], [300,600,1200],
      [740,988,1318], [392,784,1176], [600,900,600,1200], [330,495,660],
      [880,660,880,1320], [523,659,784,1047], [350,700,1050,1400], [500,750,1000],
      [294,588,882,1176], [700,840,1050,1400], [420,630,840,1260], [260,520,780,1040]
    ];
    const notes=patterns[n];

    notes.forEach((freq,i)=>{
      const o=c.createOscillator(), g=c.createGain();
      o.type = n%3===0 ? "square" : (n%3===1 ? "sawtooth" : "triangle");
      o.frequency.setValueAtTime(freq,now+i*.10);
      o.frequency.exponentialRampToValueAtTime(freq*1.03,now+i*.10+.08);
      const t=now+i*.10;
      g.gain.setValueAtTime(.0001,t);
      g.gain.exponentialRampToValueAtTime(.55/(1+i*.12),t+.012);
      g.gain.exponentialRampToValueAtTime(.0001,t+.20);
      o.connect(g); g.connect(master);
      o.start(t); o.stop(t+.23);
    });

    // Extra "hit" layer makes the winner sound more obvious on phone speakers.
    const hit=c.createOscillator(), hg=c.createGain();
    hit.type="triangle";
    hit.frequency.setValueAtTime(110,now);
    hit.frequency.exponentialRampToValueAtTime(55,now+.18);
    hg.gain.setValueAtTime(.0001,now);
    hg.gain.exponentialRampToValueAtTime(.45,now+.01);
    hg.gain.exponentialRampToValueAtTime(.0001,now+.20);
    hit.connect(hg); hg.connect(master);
    hit.start(now); hit.stop(now+.22);

    setTimeout(()=>c.close(),1400);
  }catch(e){}
}
...
function teamPage(){
 app.innerHTML=`<div class="wrap"><section class="card"><span class="pill">ROOM ${room}</span><h2>Nama & Suara Tim</h2><p class="muted">Pilih suara yang akan terdengar saat tim Anda memenangkan bel.</p><input id="name" maxlength="25" placeholder="Contoh: GARUDA" autocomplete="off"><div id="sounds" class="soundgrid"></div><div class="buttons"><button id="preview" class="secondary">▶ Coba Suara</button><button id="go">Masuk ke Permainan</button></div></section></div>`;
 let sg=$("#sounds");sg.innerHTML=SOUNDS.map((s,i)=>`<button class="sound ${i+1===soundId?"active":""}" data-id="${i+1}"><span class="ico">${s[0]}</span><b>${s[1]}</b></button>`).join("");
